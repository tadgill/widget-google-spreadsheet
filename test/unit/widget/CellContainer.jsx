...
describe("<CellContainer />", function() {
  ...
  describe("<Cell />", function() {
    ...
    it("Should use className from columnFormats prop", function() {
      ...
      expect(wrapper.find(Cell).props().className).to.equal(columnFormats[2].id);
    });

    it("Should add 'green' class for a positive number when condition is 'value-positive'", function() {
      columnFormats = [
        { width: 100 },
        { id: "B", width: 200, numeric: true, colorCondition: "value-positive" },
        { width: 300 }
      ];
      ...
      expect(wrapper.find(Cell).props().className).to.equal(columnFormats[1].id + " green");
    });
    ...
    it("Should add 'red' class for a negative number when condition is 'value-positive'", function() {
      columnFormats = [
        { width: 100 },
        { width: 200 },
        { id: "C", width: 300, numeric: true, colorCondition: "value-positive" }
      ];

      wrapper = mount(<CellContainer data={"-5"} mainClass={mainClassName} columnKey={2}
                                     width={100} height={50} columnFormats={columnFormats} />);

      expect(wrapper.find(Cell).props().className).to.equal(columnFormats[2].id + " red");
    });
    ...
    it("Should add 'red' class for a positive number when condition is 'value-negative'", function() {
      columnFormats = [
        { width: 100 },
        { id: "B", width: 200, numeric: true, colorCondition: "value-negative" },
        { width: 300 }
      ];
      ...
      expect(wrapper.find(Cell).props().className).to.equal(columnFormats[1].id + " red");
    });
    ...
    it("Should add 'green' class for a negative number when condition is 'value-negative'", function() {
      columnFormats = [
        { width: 100 },
        { width: 200 },
        { id: "C", width: 300, numeric: true, colorCondition: "value-negative" }
      ];

      wrapper = mount(<CellContainer data={"-5"} mainClass={mainClassName} columnKey={2}
                                     width={100} height={50} columnFormats={columnFormats} />);

      expect(wrapper.find(Cell).props().className).to.equal(columnFormats[2].id + " green");
    });
    ...
    it("Should not add color condition class if value is not a number", function() {
      ...
      expect(wrapper.find(Cell).props().className).to.equal("A");
    });
